...
read_csv = "spec_tbl_df" "tbl_df"      "tbl"         "data.frame" and read_csv2 = "spec_tbl_df" "tbl_df"      "tbl"         "data.frame"
...
kings <- read_csv2("data/Kongerækken-csv.csv")
...
kings_ny<- kings %>%
  ...
  mutate(mid_year=`Regerings start`+duration/2)
...
```{r eval = TRUE}
mean(kings_ny$duration, na.rm=TRUE)
```
Resultat = 19.93
...
```{r eval = TRUE}
avg_duration<- mean(kings_ny$duration, na.rm=TRUE)

long_reign_kings <- kings_ny %>%
  filter(duration > avg_duration)
long_reign_kings %>%
  count()
# Svar = 27 har regeret længere end gennemsnittet
