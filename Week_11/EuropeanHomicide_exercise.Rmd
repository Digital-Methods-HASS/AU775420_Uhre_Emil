...
ggplot(data = Western_Europe)+
  geom_line(aes(x=Year,y=homicides_per_100k,colour = Entity))
...
```{r Plot homicides 3, eval = TRUE}
kongerækken <- read_csv2("data/kongerækken-csv.csv")
kongerækken <- mutate(kongerækken, Midtår = kongerækken$`Regering slut`-(kongerækken$`Regering slut`- kongerækken$`Regerings start`)/2)
...
ggplot(data = Western_Europe) +
  ...
  labs(title = "Long term trend in homicides",
       x = "Year",
       y = "Homicides per 100,000 inhabitants",
       colour = "Country") +  # Change the legend title to "Country"
...
The visualizations show that the number of homicides has decreased in Western Europe from the Middle Ages to the 20th century. This suggests that society has become more peaceful and less violent over time, which could mean we are more civilized now. The drop in murder rates is due to several reasons. First, our laws and police forces have gotten better at dealing with crime and keeping order. This has made our societies more stable and safe.

But there are other factors too. Better healthcare has helped people live longer and healthier lives, which might reduce violent situations. Also, societal norms and values have changed. We now place more importance on human rights, education, and social welfare. This has created a culture where we respect each other's lives and resolve conflicts without violence.

Economic development and urbanization have also changed our societies. When people become more interdependent and cooperative, there's less reason to be violent. Democratic institutions and the rule of law have given us peaceful ways to solve problems, which has also helped reduce violence.

While these changes are positive, we should remember that other forms of violence still exist. For example, domestic violence and cybercrime might have become more common in recent decades. So, even though we can be happy about the decrease in murder rates.